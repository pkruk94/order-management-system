...
public class UpdateWarrantyValidator extends AbstractValidator<UpdateWarranty> {

    private static final Integer MINIMAL_COMPLETION_TIME_IN_DAYS = 1;
    private static final Integer MINIMAL_DURATION_IN_YEARS = 1;

    @Override
    public Map<String, String> validate(UpdateWarranty item) {
        ...
        if (item.getId() == null) {
            errors.put("id", "warranty id cannot be null");
            return errors;
        }
